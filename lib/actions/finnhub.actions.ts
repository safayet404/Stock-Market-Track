...
// Profile shape from Finnhub /stock/profile2
...
// Extend the FinnhubSearchResult with an internal optional exchange
  ...
  async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
      ...
      if (!trimmed) {
        ...
        // Map into the extended type (with optional __exchange)
        results = Array.isArray(data?.result)
          ? data.result.map(
              (r): FinnhubSearchResultWithExchange => ({
                ...r,
              })
            )
          : [];
